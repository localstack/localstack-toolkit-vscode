...
> Source: in-editor feedback. Two threads: (1) compact the Resources tree by merging the service and resource-type levels into one row, and filter views at that finer granularity; (2) render Resource Details as a webview table with per-field-type formatting (clickable interactions deferred).

### 20a. Merge service + resource-type into one Resources row

- [x] 20a.1 Replace `ResourceServiceTreeItem` and `ResourceTypeTreeItem` with a single `ResourceServiceTypeTreeItem` (carries `parent` region, `service` focus, `provider`, and `resourceType` focus): `label = provider.getName()`, `description = pluralName` (the type, in dimmed text), `iconPath = provider.getIconPath(provider.getId())`, collapsible `Collapsed` (so ARNs are not auto-fetched until the row is expanded). One row per (service, resource-type) pair — multi-type services (Lambda, Step Functions) render multiple sibling rows sharing the service name + icon
- [x] 20a.2 Remove the service icon from `ResourceArnTreeItem` (drop its `iconPath` constructor param) so resource leaves inherit the blank icon; the service icon now lives only on the merged row
- [x] 20a.3 In `resources/viewProvider.ts`, fuse `makeResourceServices` + `makeResourceTypes` into `makeResourceServiceTypes(region)` that emits one merged row per (service, resource-type) — expanding the wildcard service to all providers and each provider to its resource types; update `getChildren` (region → merged rows → ARNs) and `makeResourceArns` to read `profile`/`region`/`provider`/`resourceType` off the shortened parent chain (drop the service-icon computation). Keep showing empty `[ No Resources ]` blocks
- [x] 20a.4 Update the resource-selection handler in `resource-browser.ts` for the shortened parent chain (`item.parent.parent.parent.profile.id`)

### 20b. Views select service/resource-type pairs

- [x] 20b.1 Change `SavedFilter` in `settings.ts`: replace `services: string[]` with `resources: { service: string; resourceType: string }[]` (a flat pair list mirroring the merged-row granularity)
- [x] 20b.2 `runFilterWizard` step 2: list one pickable item per (service, resource-type) pair across all providers (label `"<Service> — <Plural Type>"`), pre-checked from `existing.resources`; require ≥1; return the picked pairs as `resources`
- [x] 20b.3 `makeFilterFocus` in `localstack/viewProvider.ts`: build the focus from the chosen pairs, grouping pairs by service into the focus shape (drop the old "every resource type of each chosen service" expansion and the now-unused `ProviderFactory` use)
- [x] 20b.4 Update the `localstack.cloudProfiles.filters` JSON schema in `package.json`: the per-filter `services` array becomes a `resources` array of `{ service, resourceType }` objects (pre-release shape change; no migrator — stale saved views are re-created)

### 20c. Resource Details as a webview table

- [x] 20c.1 Convert `localstack.resourceDetails` to a webview: set `"type": "webview"` on the view contribution in `package.json`; register it via `window.registerWebviewViewProvider` in `resource-browser.ts` (replacing the `createTreeView`); keep the `setArn` selection wiring and the `refreshResourceDetails` command
- [x] 20c.2 Rewrite `ResourceDetailsViewProvider` as a `vscode.WebviewViewProvider`: store the resolved view + current `arn`/`profile`; render a self-contained HTML key/value table (CSP `default-src 'none'; style-src 'unsafe-inline'`, no scripts; theme via `--vscode-*` CSS variables; HTML-escaped values); show a placeholder when nothing is selected and an error row when `describeResource` throws; `setArn`/`refresh` re-render (re-fetching `describeResource`)
...
> Source: in-editor feedback. The blanket "blank icon by default" (18.1) no longer makes sense — after the merged-row change (20a) most Resources rows have no icon, so defaulting them to a blank only indents the whole tree into an empty icon column, widening it. Revert the blanket default but KEEP the one `blank` that aligns the instance's `View: All Resources` selector with the `App Inspector` node (the original 17.4 intent).
